...
func (g *schemaGenerator) generateDeclaredType(
	...
	baseTypeName := scope.string()
	theType, err := g.generateType(t, scope)
	id := ""
	switch theType.(type) {
	case codegen.PrimitiveType:
		id = theType.(codegen.PrimitiveType).Id
	}
	if err != nil {
		return nil, err
	}
	typeInterface, ok := theType.(interface{})
	if !ok {
		return nil, fmt.Errorf("expected type to be an interface, got %T", theType)
	}

	existingDecl, _ := g.output.findExistingTypeDecl(baseTypeName, &typeInterface)

	if existingDecl != nil {
		return &codegen.NamedType{Decl: existingDecl}, nil
	}

	decl := codegen.TypeDecl{
		Name:    g.output.uniqueTypeName(baseTypeName, &typeInterface),
...
func (g *schemaGenerator) generateEnumType(
	...
	enumId := strings.Join(enumValues, ",")

	var theType interface{} = codegen.TypeDecl{
		Name: scope.string(),
		Type: enumType,
		Id:   enumId,
	}

	existingDecl, _ := g.output.findExistingTypeDecl(scope.string(), &theType)
	...
	enumDecl := codegen.TypeDecl{
		Name: g.output.uniqueTypeName(scope.string(), &theType),
		Type: enumType,
		Id:   enumId,
	}
