...
```bash
# Type checking
mypy src/langtools/package_name/
...
3. **Type checking**: `mypy src/ --show-error-codes`
...
Based on the directory structure, `langtools-ai/` is the only currently implemented package following the new architecture. Use this as a reference for implementing the other packages.